...
function transformToLis(obj) {
  ...
  for (let i = 0; i < keyArr.length; i++) {
    let str = `<li>${keyArr[i]}: ${valArr[i]}</li>`;
    cleanArr.push(str);
  }
  return cleanArr;
};
...
//   let totalSum = 0;
//   for (let i in matrix) {
//     let arrSum = matrix[i].reduce((runningTotal, currentNumber) => {
//       return parseInt(runningTotal + currentNumber);
//     }, 0);
//     totalSum += arrSum;
//   }
//   return totalSum;
...
const totalSum = (input) => {
  let totalSum = 0;
  for (let i in input) {
    let arrSum = input[i].reduce((acc, currentNum) => {
      return parseInt(acc + currentNum);
    }, 0);
    totalSum += arrSum;
  }
  return totalSum;
...
const divisibleByFiveTwoToThePower = (input) => {
  let finalArray = [];
  for (let i in input) {
    let newArr = input[i].filter(value => (typeof value === 'number' && value % 5 === 0));
    let arrayTwoToThePower = newArr.map((value => {
      return Math.pow(2, value);
    }));
    finalArray.push(arrayTwoToThePower);
  }
  return finalArray;
};
...
let findMaleAndFemale = (data) => {
  let nameArr = data.reduce((acc, value) => {
...
to return the name of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {
  // Solution code here...
  // let shortestToTallest = data.sort((a, b) => a.height - b.height);
  // return shortestToTallest[0].name;
