...
const toLastNames = people => {
  // Solution code here...
  let newArr = people.map(value => `${value.firstName} ${value.lastName}`);
  return newArr;
};
...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((runningTotal, currentObj) => {
    return parseInt(runningTotal + currentObj.purchasePrice);
  }, 0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  arr.reduce((newArr, characterObj) => {
    return newArr.push(characterObj.name);
  }, []);
};
